...
# 【极简版数据库配置】
# 移除了原有的 weibo_table 和 zhihu_table 映射。
# 表名将由 mysql_client.py 内部通过传入的 platform 参数（如 'weibo'）动态推导出 weibo_base, weibo_trend 等。
MYSQL_CONFIG = {
    'host': '127.0.0.1',
    'port': 3306,
    'user': 'root',
...
# Kafka 配置 (这里的 topic 是指真正的消息队列 Topic，保持不变)
KAFKA_CONFIG = {
